失礼いたしました。Markdown（.md）ファイルの中にコードブロック（```）が入れ子になっているため、表示が途中で終了して見えていたようです。

GitHubの編集画面に**そのまま丸ごと貼り付ければ、正しく表示される形式**で作成しました。以下の枠内をすべてコピーしてください。
...
これをGitHubの `README.md` に貼り付けて保存（Commit）すれば、セットアップ方法の中のコードも、その後の「データの仕組み」などのセクションも、すべて綺麗に表示されます！

URLの書き換えだけ最後にお忘れなく！他に修正したい部分はありますか？
